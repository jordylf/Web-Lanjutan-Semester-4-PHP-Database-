...
function tambahBuku($dataBuku)
{
  ...
  $queryInsertDataBuku = "INSERT INTO buku (cover, id_buku, kategori, judul, pengarang, penerbit, tahun_terbit, jumlah_halaman, buku_deskripsi) VALUES ('$cover', '$idBuku', '$kategoriBuku', '$judulBuku', '$pengarangBuku', '$penerbitBuku', '$tahunTerbit', $jumlahHalaman, '$deskripsiBuku')";
...
{
  $searchMember = "SELECT * FROM member WHERE nisn LIKE '%$keyword%' OR kode_member LIKE '%$keyword%' OR nama LIKE '%$keyword%' OR jurusan LIKE '%$keyword%'";
...
function updateBuku($dataBuku)
{
  ...
  $queryUpdate = "UPDATE buku SET cover = '$cover', kategori = '$kategoriBuku', judul = '$judulBuku', pengarang = '$pengarangBuku', penerbit = '$penerbitBuku', tahun_terbit = '$tahunTerbit', jumlah_halaman = '$jumlahHalaman', buku_deskripsi = '$deskripsiBuku' WHERE id_buku = '$idBuku'";
...
function pinjamBuku($dataBuku)
{
  ...
  if (mysqli_num_rows($nisnResult) > 0) {
    echo "<script>alert('Anda sudah meminjam buku, Harap kembalikan dahulu buku yg anda pinjam!');</script>";
    return 0;
  }

  $queryPinjam = "INSERT INTO peminjaman (id_buku, nisn, id_admin, tgl_peminjaman, tgl_pengembalian) VALUES ('$idBuku', '$nisn', '$idAdmin', '$tglPinjam', '$tglKembali')";
...
function pengembalian($dataBuku)
{
  ...
  $denda = htmlspecialchars($dataBuku["denda"]);
  ...
  $queryPengembalian = "INSERT INTO pengembalian (id_peminjaman, id_buku, nisn, id_admin, buku_kembali, keterlambatan, denda) VALUES ('$idPeminjaman', '$idBuku', '$nisn', '$idAdmin', '$bukuKembali', '$keterlambatan', '$denda')";
...
function bayarDenda($data)
{
  ...
  $jmlDenda = htmlspecialchars($data["denda"]);
  $jmlDibayar = htmlspecialchars($data["bayarDenda"]);
